...
class Vias(BaseResource):
    """Model's collection routing (controller)."""

    @marshal_with(model_fields)
    def get(self):
        """Get all entries."""
        entries = self.db.session.query(ViaModel).all()
        return entries
    ...
    def post(self):
        ...
        existing_entry = (
            self.db.session.query(ViaModel).filter_by(codigo=args["codigo"]).first()
        )
        if existing_entry:
            return abort(422, message="Já existe uma Via com esse código.")

        entry = ViaModel(nome=args["nome"], codigo=args["codigo"])
...
class Via(BaseResource):
    """Model's routing (controller)."""

    @marshal_with(model_fields)
    def get(self, id):
        entry = self.db.session.query(ViaModel).filter_by(id=id).first()
        if not entry:
